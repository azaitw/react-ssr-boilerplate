...
process.env.apiHost = (process.env.NODE_ENV === 'production')
  ? 'https://PROD-DOMAIN'
  : 'http://DEV-DOMAIN'
...
app.get('*', (req, res) => {
  ...
  const render = ({ pageObj, result }) => {
    return fs.readFile(path.join(__dirname, '../dist/index.html'), 'utf8', (err, data) => {
      ...
      // Insert rendered html, style tag, and fetched data (save as window.INIT_DATA) into html page
  ...
  if (pageObj.page === 'home') {
    ...
    // Redirect to home if data not available
    api.getItem()
      .then(result =>
        (result)
          ? render({ pageObj, result })
